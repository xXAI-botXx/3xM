...
def compare_materials_and_copy(material_folder, compare_folder, similarity_threshold=0.9):

    materials = {}
    for cur_root_path, cur_dirs, cur_files in os.walk(material_folder):
        if len(cur_files) > 2:
            material_name = cur_root_path.split("/")[-2]
            material_path = "/".join(cur_root_path.split("/")[:-1])
            material_textures = ['normal.jpg', 'color.jpg', 'metal.jpg', 'roughness.jpg']
            textures = {}
            for texture_name in material_textures:
                texture_path = os.path.join(cur_root_path, texture_name)
...
if __name__ == "__main__":
    material_folder = "/home/tobia/data/3xM/final/materials"  # Path to your materials folder
    compare_folder = "/home/tobia/data/3xM/materials/compare"  # Path to the comparison folder
    ...
    compare_materials_and_copy(material_folder, compare_folder, similarity_threshold)
